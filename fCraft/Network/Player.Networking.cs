...
namespace fCraft {
    ...
    public sealed partial class Player {
        ...
        bool LoginSequence() {
            ...
            {
                if (!File.Exists("MojangAccounts.xml"))
                {
                    Logger.Log(LogType.Error, "Warning, MojangAccounts.xml not found! Creating default file.");
                   
                    //if the xml file is corrupted or missing, create a new, default one
                    XmlWriterSettings settings = new XmlWriterSettings();
                    settings.Indent = true;

                    XmlWriter writer = XmlWriter.Create("MojangAccounts.xml", settings);
                ...
                {
                    String[] mojangAccounts = File.ReadAllLines("MojangAccounts.xml");
                    string mojangAccString = String.Join(Environment.NewLine, mojangAccounts);

                    int lineCount = File.ReadLines("MojangAccounts.xml").Count();     
                    int x = lineCount - 2;
                    if (!mojangAccString.Contains(splitGivenName[0]))
                    {
                       //if user is not in xml file, add them to file
                       var xdoc = XDocument.Parse(@mojangAccString);
                       xdoc.Root.Add(
                       new XElement(splitGivenName[0],
                       new XAttribute("value", x)));
                       xdoc.Save("MojangAccounts.xml");
                    }                                                                     
                }
                //load the xml file for the user
                XmlReader xmlLoader = XmlReader.Create("MojangAccounts.xml");
                while (xmlLoader.Read())
                {
                    if (xmlLoader.NodeType == XmlNodeType.Element)
                    {
                        if (xmlLoader.Name == splitGivenName[0])
                        {
                            emailName = splitGivenName[0] + "@" + xmlLoader.GetAttribute(0);
                            Logger.Log(LogType.SystemActivity, emailName);
                        }
                    }
                    xmlLoader.Close();
                }
